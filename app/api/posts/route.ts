...
// Static Export 빌드 시에는 무시됩니다.

// 프로덕션 빌드에서 이 파일이 실행되지 않도록 함
if (process.env.NODE_ENV === 'production' && typeof window === 'undefined') {
  // 빌드 시에는 아무것도 export 하지 않음
} else {
  const fs = require('fs');
  const path = require('path');
  const matter = require('gray-matter');
  const { getAllPostSlugs } = require('@/lib/blog/markdown');
...
// Static Export 빌드를 위한 더미 export
